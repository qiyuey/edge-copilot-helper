...
pub fn install() -> Result<()> {
    ...
    if let Some(parent) = plist_path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("Failed to create LaunchAgents directory: {:?}", parent))?;
    }
    ...
    println!();
    println!("Monitor with: tail -f {:?}/service.log", log_dir);
...
pub fn uninstall() -> Result<()> {
    ...
    // 4. Remove logs
    if log_dir.exists() {
        println!("Removing logs: {:?}", log_dir);
        fs::remove_dir_all(&log_dir).with_context(|| format!("Failed to remove {:?}", log_dir))?;
    }
